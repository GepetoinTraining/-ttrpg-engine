...
// TweaksPanel — ported from the Claude Design prototype.
// The original communicated with a design-tool host via postMessage; the host
// would activate / deactivate / persist edits. Outside that host we just keep
// state local and expose a floating button to open the panel.
...
const TWEAKS_STYLE = `
  .twk-toggle-btn{position:fixed;right:16px;bottom:16px;z-index:2147483645;
    ...
    background:rgba(31,27,22,.85);color:#f4efe4;
    width:40px;height:40px;border-radius:999px;
    font:600 14px/1 ui-sans-serif,system-ui,sans-serif;letter-spacing:.04em;
    box-shadow:0 6px 20px rgba(0,0,0,.25)}
  .twk-toggle-btn:hover{background:rgba(31,27,22,1)}
...
export function TweaksPanel({
  ...
  const offsetRef = React.useRef({ x: 16, y: 64 })
  const PAD = 16
