...
export class NaturalTopology {
  ...
  generateLots(streets: Street[], count: number): Lot[] {
    ...
    const minLotSpacing = 16  // Must exceed max polygon radius to prevent overlap
    let attempts = 0
    const maxAttempts = count * 20

    while (centers.length < count && attempts < maxAttempts) {
      attempts++
      const candidate: Point = { x: this.rng.range(8, 92), y: this.rng.range(8, 92) }
      ...
      if (!tooClose) {
        for (const other of centers) {
          if (Math.hypot(candidate.x - other.x, candidate.y - other.y) < minLotSpacing) {
            tooClose = true
            break
          }
        }
    ...
    for (let i = 0; i < centers.length; i++) {
      ...
      const size = this.rng.range(5, 10)
