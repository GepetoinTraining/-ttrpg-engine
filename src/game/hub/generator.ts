...
// L4 hex direction vectors (flat-top, same order as L0 roads)
const HEX_DIRS: { q: number; r: number }[] = [
  { q: 1, r: 0 },   // 0: E
  { q: 1, r: -1 },  // 1: NE
  { q: 0, r: -1 },  // 2: NW
  { q: -1, r: 0 },  // 3: W
...
/** Walk a straight L4 hex line from origin in a direction for `len` steps. */
function hexLine(dir: { q: number; r: number }, startDist: number, endDist: number): { q: number; r: number }[] {
  const pts: { q: number; r: number }[] = []
  for (let i = startDist; i <= endDist; i++) {
    pts.push({ q: dir.q * i, r: dir.r * i })
  }
  return pts
}

/** Create a ring of L4 hexes at a given radius from origin. */
...
export class HubGenerator {
  static generate(seed: HubSeed, worldNodeId: string): Hub {
    ...
    const CHUNK_L4 = 7  // L4 hexes per hub chunk
    const coreRadiusL4 = Math.ceil(gridSize * CHUNK_L4 / 2) + 2
    const territoryRadiusL4 = coreRadiusL4 + 4
    const influenceRadiusL4 = territoryRadiusL4 + 8

    // ─── Gate-to-Center Road Network ───
    const gateDirs = seed.gateDirs.length > 0 ? seed.gateDirs : [0]  // default: east
    const hubRoads: { q: number; r: number }[][] = []

    for (const gateIdx of gateDirs) {
      const dir = HEX_DIRS[gateIdx % 6]
      // Road from center (0,0) out to core edge
      const road = hexLine(dir, 0, coreRadiusL4)
      hubRoads.push(road)
    }

    // ─── District Layout (along roads) ───
    ...
    // ─── Extraction Camps ───
    const extractionCamps = HubGenerator.generateCamps(
      seed.biomeType, gateDirs, influenceRadiusL4, coreRadiusL4, rng, seed.worldNodeId
    )
    ...
    // ─── Economy (influenced by biome) ───
    const economyType = HubGenerator.getEconomyFromBiome(seed.biomeType, rng)
    ...
    return {
      ...
      economy: {
        type: economyType,
        wealthLevel: 'modest',
        exports: extractionCamps.map(c => c.output[0]).filter(Boolean),
        imports: [],
  ...
  /** Generate extraction camps based on biome resources. */
  private static generateCamps(
    biomeType: string,
    gateDirs: number[],
    ...
    const camps: ExtractionCamp[] = []

    // Camp types by biome
    const BIOME_CAMPS: Record<string, { type: CampType; resource: string; output: string[]; count: number }[]> = {
      forest:       [{ type: 'logging', resource: 'oak_tree', output: ['oak_logs', 'firewood'], count: 2 },
                     { type: 'hunting', resource: 'deer', output: ['venison', 'hide'], count: 1 }],
      dense_forest: [{ type: 'logging', resource: 'ancient_oak', output: ['hardwood_logs', 'timber'], count: 3 },
                     { type: 'herbalism', resource: 'rare_herbs', output: ['potion_ingredients'], count: 1 }],
      hills:        [{ type: 'mining', resource: 'copper_ore', output: ['copper_ore', 'tin_ore'], count: 2 },
                     { type: 'quarry', resource: 'limestone', output: ['stone_blocks'], count: 1 }],
      mountains:    [{ type: 'mining', resource: 'iron_ore', output: ['iron_ore', 'stone'], count: 3 }],
      plains:       [{ type: 'farm', resource: 'grain_grass', output: ['grain', 'hay'], count: 3 }],
      coast:        [{ type: 'fishing', resource: 'fish', output: ['fish', 'salt'], count: 2 }],
      swamp:        [{ type: 'herbalism', resource: 'swamp_moss', output: ['alchemical_reagents'], count: 1 },
                     { type: 'hunting', resource: 'frog', output: ['alligator_hide', 'swamp_oil'], count: 1 }],
      desert:       [{ type: 'quarry', resource: 'sandstone', output: ['sandstone_blocks'], count: 1 }],
      tundra:       [{ type: 'hunting', resource: 'reindeer', output: ['fur', 'meat'], count: 2 }],
    }

    const campDefs = BIOME_CAMPS[biomeType] || BIOME_CAMPS['plains']
    const campDist = Math.floor((influenceRadius + coreRadius) / 2) + 2  // halfway between core and influence

    for (const def of campDefs) {
      for (let i = 0; i < def.count; i++) {
        // Place camp in a semi-random direction, biased toward gate roads
        const baseAngle = (gateDirs.length > 0)
          ? (gateDirs[i % gateDirs.length] * 60 + rng.range(-40, 40)) * Math.PI / 180
          : rng.next() * Math.PI * 2

        // Offset perpendicular to gate direction for spread
        const spreadAngle = baseAngle + rng.range(-0.8, 0.8)
        const dist = campDist + rng.rangeInt(-2, 3)

        // Convert angle to hex coords (approximate)
        const px = Math.cos(spreadAngle) * dist
        const py = Math.sin(spreadAngle) * dist
        const q = Math.round(px - py / Math.sqrt(3))
        const r = Math.round(py * 2 / Math.sqrt(3))

        camps.push({
          id: `camp_${def.type}_${i}_${rng.rangeInt(100, 999)}`,
          type: def.type,
          position: { q, r },
          resource: def.resource,
          workers: rng.rangeInt(3, 12),
          output: def.output,
          seed: `${worldNodeId}_camp_${def.type}_${i}`,
        })
      }
    }
  ...
  /** Determine economy type from biome. */
  private static getEconomyFromBiome(biomeType: string, rng: SeededRNG): Hub['economy']['type'] {
    const BIOME_ECONOMY: Record<string, Hub['economy']['type'][]> = {
      forest:       ['craft', 'trade'],
      dense_forest: ['craft'],
      hills:        ['mining', 'craft'],
      mountains:    ['mining'],
      plains:       ['agricultural', 'trade'],
      coast:        ['fishing', 'trade'],
      swamp:        ['fishing'],
      desert:       ['trade', 'military'],
      tundra:       ['military', 'trade'],
    }
    const options = BIOME_ECONOMY[biomeType] || ['trade']
